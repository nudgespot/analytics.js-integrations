...
describe('Extole', function(){
  ...
  describe('after loading', function(){
    ...
    describe('#conversionEvents', function(){
      beforeEach(function(){
        extole.options.conversionEvents = {
          'loan made' : {
            'e' : 'email',
            'tag:cart_value' : 'loan',
      ...
      it('should not track a Completed Order', function(){
      ...
      it('should track an event in conversionEvents', function(done){
        var randomOrderId = Math.floor(Math.random() * 999999);
      ...
      it('should track a different event in conversionEvents', function(done){
        var randomOrderId = Math.floor(Math.random() * 999999);
        var user = analytics.user();
  ...
  var loadXtlMicrosite = function() {
  	env = "pr",
  		clientIdZoneName = "99286621|microsite1".split("|"), // This should be put in by nginx.
  		clientId = clientIdZoneName[0],
  		zoneName = clientIdZoneName[1],
  		config = {
  			lo: {
  		...
  		MEDIA_SERVER = config[env].MEDIA_SERVER,
  		VERSION = "20140710.1945",
